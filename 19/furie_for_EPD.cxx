...
TProfile *acceptL  =(TProfile*)profile1->Get("acceptL");
TProfile *Anty_accept_L  =(TProfile*)profile1->Get("accept_barL");
TFile *f_l_out=new TFile("furie_l.root","RECREATE");
TFile *f_antyl_out=new TFile("furie_antyl.root","RECREATE");
 TProfile *accL  =(TProfile*)profile1->Get("acceptL");

  TProfile *accAnty_L  =(TProfile*)profile1->Get("accept_barL");
...
for(int c=0;c<9;c++)
{
 ...
 Pol[8-c]=345*fit_f->GetParameter(1)/(1.273*acceptL->GetBinContent(c+1)*sqrt(2*res->GetBinContent(c+1)));
 ePol[8-c]=345*fit_f->GetParError(1)/(1.273*acceptL->GetBinContent(c+1)*sqrt(2*res->GetBinContent(c+1)));
...
for(int c=0;c<9;c++)
{
         ...
         for(int i=0;i<10;i++)
         {
           TH1D *hist_in= Anty_Lambda->ProjectionX(Form("x_%d_%d",i,c),i+1,i+1,c+1,c+1,"e");
           hist_in->SetTitle(";m_{inv}");
           c1->cd(i+1);
           delete_bg(hist_in,sum);
           amunt[i]=sum[0];
 ...
 c2->cd();
 furie[c]->Fit(fit_f,"RME");
 Pol[8-c]=-345*fit_f->GetParameter(1)/(sqrt(2*res->GetBinContent(c+1))*1.273*Anty_accept_L->GetBinContent(c+1));
 ePol[8-c]=345*fit_f->GetParError(1)/(sqrt(2*res->GetBinContent(c+1))*1.273*Anty_accept_L->GetBinContent(c+1));
    cos_f->SetParameter(0,fit_f->GetParameter(0));
    cos_f->SetParameter(1,fit_f->GetParameter(2));
    cos_f->SetLineColor(1);
...
furie[c]->Draw("AP*");
fit_f->Draw("same");
furie[c]->SetTitle(Form("cent_%d_anty",c));
sin_f->Draw("same");
 cos_f->Draw("same");
 c2->BuildLegend();
    f_antyl_out->cd();
...
TGraphErrors *Polar_anty=new TGraphErrors(9,cent,Pol,ecent,ePol);
    Polar_anty->SetTitle("Analysis for #bar{#Lambda};centrality (%); Polarisation (%)");
    Polar_anty->SetMarkerStyle(8);
    f_antyl_out->cd();
Polar_anty->Write();   
 f_antyl_out->Close();
