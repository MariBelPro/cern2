...
TProfile *acceptanceL=new TProfile("acceptKsi_l","acceptL",9,-0.5,8.5);
TProfile *acceptance_barL=new TProfile("accept_barKsi_l","accept_barL",9,-0.5,8.5);
 ...
 for(int i=0; i<dst->numberOfKFPs(); i++)
     {
       ...
       TVector3 b;
        ...
        StFemtoKFP *doter_p, *lambda_kfp, *anty_lambda_kfp;
       
      
        //for limbda
        if (kfp->pdg()==3312)
        {
          ksi=kfp->fourMomentum();
          lambda_kfp=dst->kfparticle(kfp->daughterIds()[1]);
          doter_p=dst->kfparticle(lambda_kfp->daughterIds()[1]);
          lambda=lambda_kfp->fourMomentum();
          ...
          if(doter_p->pdgId()==2212)
          {
            ...
            Double_t s1=proton_hell->pathLength(lambda_kfp->decayPoint());
            proton.SetVectM(proton_hell->momentumAt(s1,Event->magneticField()/pow(10,14)),0.938272);
            proton.Boost(-b);
             if(isnan( proton.Theta())) continue;
            acceptanceL->Fill(Event->cent9(),sin( proton.Theta()));
           Totol_of_lambda->Fill(lambda_kfp->fourMomentum().M(), Range(information.FullPhiWeightedAndShiftedPsi(1)-proton.Phi()),Event->cent9());
        ...
        if (kfp->pdg()== -3312)
        {
          ksi_plus=kfp->fourMomentum();
          anty_lambda_kfp=dst->kfparticle(kfp->daughterIds()[1]);
          doter_p=dst->kfparticle(anty_lambda_kfp->daughterIds()[0]);
          anty_lambda=anty_lambda_kfp->fourMomentum();
          b= anty_lambda.BoostVector();
          if(doter_p->pdgId()==-2212)
          {
            StFemtoPhysicalHelix *proton_hell=new StFemtoPhysicalHelix(doter_p->p(),doter_p->decayPoint(),Event->magneticField()/pow(10,14),-1 );
            Double_t s1=proton_hell->pathLength(anty_lambda_kfp->decayPoint());
            anty_proton.SetVectM(proton_hell->momentumAt(s1,Event->magneticField()/pow(10,14)),0.938272);
            anty_proton.Boost(-b);
               if(isnan( anty_proton.Theta())) continue;
            acceptance_barL->Fill(Event->cent9(),sin(anty_proton.Theta()));
            Totol_of_Antylambda->Fill(anty_lambda_kfp->fourMomentum().M(),Range(information.FullPhiWeightedAndShiftedPsi(1)-anty_proton.Phi()),Event->cent9());
          }
