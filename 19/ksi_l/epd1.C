...
void epd1(const Char_t *inFile = "olo.list",
 ...
 for(int i=0; i<dst->numberOfKFPs(); i++)
     {
       ...
       TVector3 b;
        ...
        if (kfp->pdg()==3312)
        {
          ...
          doter_p=dst->kfparticle(anty_lambda_kfp->daughterIds()[1]);
        ...
        if (kfp->pdg()== -3312)
        {
          ...
          anty_lambda_kfp=anty_lambda_kfp->fourMomentum();
            b= anty_lambda_kfp.BoostVector();
          if(doter_p->pdgId()==-2212)
          {
            StFemtoPhysicalHelix *proton_hell=new StFemtoPhysicalHelix(doter_p->p(),doter_p->decayPoint(),Event->magneticField()/pow(10,14),-1 );
