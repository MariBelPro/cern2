...
TFile *profile1 =TFile::Open("StEpdEpFinderCorrectionHistograms_INPUT19.6.root", "READ");
 TProfile *L  =(TProfile*)profile1->Get("acceptL");

  TProfile *Anty_L  =(TProfile*)profile1->Get("accept_barL");
 ...
 Polar->SetTitle("Analysis for #Lambda (EPD);centrality (%); A_{0}");
 ...
 Polar_a->SetTitle("Analysis for #bar{#Lambda} (EPD);centrality (%); A_{0}");
Polar_a->Draw("P"); 
c2->BuildLegend();
