...
TFile *profile1 =TFile::Open("StEpdEpFinderCorrectionHistograms_INPUTeta.root", "READ");
...
//gStyle->SetOptStat(kFALSE);
 ...
 Double_t Pol[5]; 
     Double_t ePol[5]; 
     Double_t cent[5]={-0.7,-0.3,0.1,0.5,0.9};  //-1,-0.6,-0.2,0.2,0.6,1
     Double_t ecent[5]={0,0,0,0,0};
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
...
for(int c=0;c<5;c++)
{
 ...
 Pol[c]=345*fit_f->GetParameter(1)/sqrt(2*res->GetBinContent(7));
 ePol[c]=345*fit_f->GetParError(1)/sqrt(2*res->GetBinContent(7));
...
furie[c]->Write();
 ...
 TGraphErrors *Polar=new TGraphErrors(5,cent,Pol,ecent,ePol);
    Polar->SetTitle("Analysis for #Lambda;#eta ; Polarisation (%)");
...
for(int c=0;c<5;c++)
{
 ...
 c2->cd();
 furie[c]->Fit(fit_f,"RME");
 Pol[4-c]=-345*fit_f->GetParameter(1)/sqrt(2*res->GetBinContent(7));
 ePol[4-c]=345*fit_f->GetParError(1)/sqrt(2*res->GetBinContent(7));
    cos_f->SetParameter(0,fit_f->GetParameter(0));
    cos_f->SetParameter(1,fit_f->GetParameter(2));
    cos_f->SetLineColor(1);
...
TGraphErrors *Polar_anty=new TGraphErrors(5,cent,Pol,ecent,ePol);
    Polar_anty->SetTitle("Analysis for #bar{#Lambda};#eta; Polarisation (%)");
    Polar_anty->SetMarkerStyle(22);
    f_antyl_out->cd();
    Polar_anty->Draw("P");
    c2->BuildLegend();
...
TLatex latex2(-0.6,3.5,"#font[12]{Au+Au #sqrt{s_{NN}}=19.6 GeV}");
