...
void epd_pt(const Char_t *inFile = "olo.list",
...
TH3D *Totol_of_lambda= new TH3D ("lambdy_tot","amunt;massa;angle;cent",200,1.105,1.125,10,-3,3,5,0.15,2);
TH3D *Totol_of_Antylambda= new TH3D ("Antylambdy_tot","amunt;massa;angle;cent",200,1.105,1.125,10,-3,3,5,0.15,2);
TH1D *lambda_pt=new TH1D("h1","h1",100,0.1,3);
...
TVector3 pVtx = Event->primaryVertex();
 ...
 if(Event->cent9()!=4 && Event->cent9()!=3 && Event->cent9()!=5) continue;
       ...
       TVector3 b;
        ...
        if (kfp->pdg()==3122)
        {
          ...
          if(doter_p->pdgId()==2212)
          {
            ...
            lambda_pt->Fill(kfp->pt());
           Totol_of_lambda->Fill(kfp->fourMomentum().M(), Range(information.FullPhiWeightedAndShiftedPsi(1)-proton.Phi()),kfp->pt());
        ...
        if (kfp->pdg()== -3122)
        {
          ...
          if(doter_p->pdgId()==-2212)
          {
            ...
            Totol_of_Antylambda->Fill(kfp->fourMomentum().M(),Range(information.FullPhiWeightedAndShiftedPsi(1)-anty_proton.Phi()),kfp->pt());
          }
