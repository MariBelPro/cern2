...
{
    TH1D *hist_out=new TH1D ("h","h",200,1.310,1.330);
    float xL;
    float xR ;
    float bg;
    float fe;
    float all;
            TF1 *full=new TF1 ("full",fit,1.310,1.330,7);
    ...
    TF1 *back_g =new TF1("bg",b_g,1.310,1.330,2);
...
//TH3D *Anty_Lambda= (TH3D*)profile1->Get("Antylambdy_tot");
 TProfile *acceptL  =(TProfile*)profile1->Get("acceptKsi");
TProfile *Anty_accept_L  =(TProfile*)profile1->Get("accept_barKsi");
...
for(int c=0;c<9;c++)
{
 ...
 Pol[8-c]=556*fit_f->GetParameter(1)/(1.273*Anty_accept_L->GetBinContent(c+1)*sqrt(2*res->GetBinContent(c+1)));
 ePol[8-c]=556*fit_f->GetParError(1)/(1.273*Anty_accept_L->GetBinContent(c+1)*sqrt(2*res->GetBinContent(c+1)));
