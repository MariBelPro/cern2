...
void delete_bg( TH1D *hist_in )
{
            ...
            full->SetParameter(6,0.001);
                 ...
                 full->SetParLimits(0,0,10000000);
                  full->SetTitle("signal + background");
            full->SetParLimits(6,0,100);
    hist_in->Fit(full,"RME");

    TF1 *back_g =new TF1("bg",b_g,1.3,1.34,2);
    ...
    TF1 *sig =new TF1("sig",signal,1.3,1.34,5);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
   //Filling furie[i]  
         ...
         for(int i=0;i<2;i++)
         {
           TH1D *hist_in= Lambda->ProjectionX(Form("x_%d_%d",1,i),6,6,i+5,i+5,"e");
           hist_in->SetTitle("distribution ;m_{inv}[GeV/c^{2}] (#bar{#Xi}^{+} #rightarrow #bar{#Lambda} + #pi^{+})");
           c1->cd(i+1);
          // hist_in->Draw();
           delete_bg(hist_in);
           c1->cd(i+1)->BuildLegend();
           TLatex latex2(1.32,200,"#font[12]{Au+Au #sqrt{s_{NN}}=19.6 GeV}");
latex2.SetTextSize(0.04);
latex2.DrawClone("Same");
           TLatex latex(1.32,400,Form("#font[12]{%d-%d centrality (%)}",70-10*i,80-10*i));
