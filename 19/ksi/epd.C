...
TH3D *Totol_of_ksi= new TH3D ("lambdy_tot","amunt;massa;angle;cent",200,1.3,1.34,10,-3,3,9,-0.5,8.5);
TH3D *Totol_of_Antyksi= new TH3D ("Antylambdy_tot","amunt;massa;angle;cent",200,1.3,1.34,10,-3,3,9,-0.5,8.5);
TProfile *acceptanceL=new TProfile("acceptKsi","acceptL",9,-0.5,8.5);
TProfile *acceptance_barL=new TProfile("accept_barKsi","accept_barL",9,-0.5,8.5);
 ...
 for(int i=0; i<dst->numberOfKFPs(); i++)
     {
       ...
       TVector3 b;
        ...
        if (kfp->pdg()==3312)
        {
          ...
          if(doter_p->pdgId()==-1)
          {
            lambda=doter_p->fourMomentum();
        lambda.Boost(-b);
        if(isnan(lambda.Theta())) continue;
            acceptanceL->Fill(Event->cent9(),sin(lambda.Theta()));
           Totol_of_ksi->Fill(kfp->fourMomentum().M(), Range(information.FullPhiWeightedAndShiftedPsi(1)-lambda.Phi()),Event->cent9());
        ...
        if (kfp->pdg()== -3312)
        {
          anty_ksi=kfp->fourMomentum();
           b= anty_ksi.BoostVector();
          doter_p=dst->kfparticle(kfp->daughterIds()[1]);
         if(doter_p->pdgId()==-1)
         {
           anty_lambda=doter_p->fourMomentum();
            anty_lambda.Boost(-b);
             if(isnan(anty_lambda.Theta())) continue;
            acceptance_barL->Fill(Event->cent9(),sin(anty_lambda.Theta()));
            Totol_of_Antyksi->Fill(kfp->fourMomentum().M(),Range(information.FullPhiWeightedAndShiftedPsi(1)-anty_lambda.Phi()),Event->cent9());
