...
void v2_TPC(const Char_t *inFile = "olo.list",
 ...
 profile1->cd();
  ...
  TProfile *v2[10];
     for(int i=0;i<10;i++)
     {
         ...
         v2[i]= new TProfile (Form("for_centrality_bin_%d",i),Form("for_centrality_bin_%d",i),12,-1,1);
     }
   
  
     TH1D *west=new TH1D("w","w",100,-3.2,3.2);         
TH1D *east=new TH1D("e","e",100,-3.2,3.2);    
...
StEpdGeom* geo = new StEpdGeom();
  ...
  for(Long64_t iEvent=0; iEvent<events2read; iEvent++) {
   ...
   TVector3 q;
    ...
    TVector3 Q_E;
    TVector3 Q_W;
    Double_t psi, phi_help2, phi_help3;
 ...
 for(Int_t iTrk=0; iTrk<nTracks; iTrk++) 
    {
      ...
      if( !isGoodTrack( femtoTrack->pMom(), femtoTrack->nHits(), femtoTrack->nHitsPoss() ) ) continue;
         ...
         if(femtoTrack->eta()>0.05) 
          {
           v2[event->cent9()]->Fill(femtoTrack->eta(),cos(order*(femtoTrack->phi()-phi_help2)),femtoTrack->pt());
           west->Fill(femtoTrack->phi()-phi_help2);
           }
            if(femtoTrack->eta()<-0.05) 
          {
            east->Fill(femtoTrack->phi()-phi_help3);
            v2[event->cent9()]->Fill(femtoTrack->eta(),cos(order*(femtoTrack->phi()-phi_help3)),femtoTrack->pt());
           }
