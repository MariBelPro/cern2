...
void  Recentring(const Char_t *inFile = "olo.list",
...
int order =3;
 ...
 TFile *profile1 =TFile::Open("Q_vector.root", "READ");
  ...
  TProfile *defined_Qxw= (TProfile*)profile1->Get("Qxw_profile");
      ...
      for(int i=0;i<9; i++)
        {
         phi_W[i]= new TH1D(Form("#phi_{W} for centrality %i",i ),Form("#phi_{W} for centrality bin %i",i ),100,0,2*TMath::Pi()/order);
         phi_W[i]->SetTitle(";#phi;");
        }  
           TH1D *phi_E[9];
      for(int i=0;i<9; i++)
        {
         phi_E[i]= new TH1D(Form("#phi_{E} for centrality %i",i ),Form("#phi_{E} for centrality bin %i",i ),100,0,2*TMath::Pi()/order);
         phi_E[i]->SetTitle(";#phi;");
        }                                                                                                         
  
  TProfile *East_flat[10];
      for(int i=0;i<10; i++)
        {
         East_flat[i]= new TProfile(Form("east_flat_%i",i ),"east_flat_",18,-0.5,17.5);
        }
  TProfile *West_flat[10];
      for(int i=0;i<10; i++)
        {
         West_flat[i]= new TProfile(Form("west_flat_%i",i),"west_flat_",18,-0.5,17.5);
        } 
...
//shift West vector
    ...
    West_flat[0]->Fill(event->cent9(),0);
    West_flat[0]->Fill(event->cent9()+9,0);
    for(int i=1;i<10;i++)
     {
       West_flat[i]->Fill(event->cent9(),sin(i*order*Q_W.Phi()));
       West_flat[i]->Fill(event->cent9()+9,cos(i*order*Q_W.Phi()));
     }
    East_flat[0]->Fill(event->cent9(),0);
    East_flat[0]->Fill(event->cent9()+9,0);
     for(int i=1;i<10;i++)
     {
       East_flat[i]->Fill(event->cent9(),sin(i*order*Q_E.Phi()));
       East_flat[i]->Fill(event->cent9()+9,cos(i*order*Q_E.Phi()));
     }

       phi_W[event->cent9()]-> Fill(GetPsiInRange(Q_W.X(),Q_W.Y(),order));
       phi_E[event->cent9()]-> Fill(GetPsiInRange(Q_E.X(),Q_E.Y(),order));
   Res->Fill(event->cent9(),cos(order*(GetPsiInRange(Q_W.X(),Q_W.Y(),order)-GetPsiInRange(Q_E.X(),Q_E.Y(),order))));
  } //for(Long64_t iEvent=0; iEvent<events2read; iEvent++)
