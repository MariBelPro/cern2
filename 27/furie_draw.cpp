...
fit_f->SetTitle("p_{0}(1+2#sum_{i=1}^{#infty}{p_{n,s}sin(n#Delta#phi)+p_{n,c}cos(n#Delta#phi))}");
fit_f->GetXaxis()->SetTitle("#Delta#phi*=#psi-#phi*_{p} (rad)");
TF1 *cos_f=new TF1("par[0]*(1+2*par[2]*cos(x))",cosn,-3.2,3.2,2); 
cos_f->SetTitle("p_{0}(1+2p_{1,c}cos(#Delta#phi))") ;
TF1 *sin_f=new TF1("par[0]*(1+2*par[1]*sin(x))",sins,-3.2,3.2,2);
sin_f->SetTitle("p_{0}(1+2p_{1,s}sin(#Delta#phi))") ;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
   //Filling furie[i]  
         ...
         for(int i=0;i<10;i++)
         {
           TH1D *hist_in= Lambda->ProjectionX(Form("x_%d_%d",i,5),i+1,i+1,5,5,"e");
...
sin_f->Draw("same");
 ...
 c1->BuildLegend();
furie[4]->Draw("P");
TLatex latex2(-2,885200,"#font[12]{Au+Au #sqrt{s_{NN}}=27 GeV}");
latex2.SetTextSize(0.04);
latex2.DrawClone("Same");

TLatex latex(-2,885200,"#font[12]{30-40% centrality}");
